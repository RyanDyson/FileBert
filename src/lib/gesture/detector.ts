...
export class GestureDetector {
  ...
  private analyzeGesture(hand: any): GestureData | null {
    ...
    if (this.previousHandX !== null) {
      ...
      const avgMovement =
        this.movementHistory.reduce((a, b) => a + b, 0) /
        this.movementHistory.length;

      if (Math.abs(avgMovement) > 3) {
    ...
    if (currentState && currentState !== this.previousHandState) {
      ...
      if (
        this.gestureSequence.length === 2 &&
        now - this.lastGestureTime > 800
      ) {
        // Open -> Closed = "Send file"
        if (
          this.gestureSequence[0] === "open" &&
          this.gestureSequence[1] === "closed"
        ) {
        ...
        if (
          this.gestureSequence[0] === "closed" &&
          this.gestureSequence[1] === "open"
        ) {
          this.lastGestureTime = now;
          this.gestureSequence = []; // Clear sequence after detection
  ...
  private isSidewaysHand(keypoints: any[]): boolean {
    ...
    const avgY =
      fingerBases.reduce((sum, p) => sum + p.y, 0) / fingerBases.length;
    const yVariance =
      fingerBases.reduce((sum, p) => sum + Math.abs(p.y - avgY), 0) /
      fingerBases.length;
    ...
    if (hasDepth) {
      const avgZ =
        fingerBases.reduce((sum, p) => sum + (p.z || 0), 0) /
        fingerBases.length;
      const zVariance =
        fingerBases.reduce((sum, p) => sum + Math.abs((p.z || 0) - avgZ), 0) /
        fingerBases.length;
